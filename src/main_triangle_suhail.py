...
for (x, y) in maskless_transformed[0]:
    if (count >= 2 and count <= 16) or count == 28:
        if count >= 7 and count <= 12:
            ...
            #print("Adding the following new chin point: ", new_chin_pt)
            #print("Replacing subdiv point: ", masked_subdiv_points[count - 2])
            masked_hull_list.append(new_chin_pt)
            masked_subdiv_points[count - 2] = new_chin_pt
            #cv2.circle(masked_cp, (int(new_chin_pt[0]), int(new_chin_pt[1])), 8, (0, 255, 255), -1)
...
print("IS THERE A DUPLICATE MASKED  :", masked_subdiv_points)
print("IS THERE A DUPLICATE MASKLESS:", maskless_subdiv_points)
